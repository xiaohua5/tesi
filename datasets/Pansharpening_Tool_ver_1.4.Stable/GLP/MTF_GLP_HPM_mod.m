...
function I_Fus_MTF_GLP_HPM = MTF_GLP_HPM_mod(I_PAN,I_MS,sensor,tag,ratio)
...
for ii = 1 : size(I_MS,3)    
  imageHR_B = (imageHR- mean2(imageHR)).*(std2(I_MS(:,:,ii))./std2(imageHR)) + ...
      mean2(I_MS(:,:,ii));  


PAN_LP = MTF(imageHR_B,sensor,ii,ratio);
...
I_Fus_MTF_GLP_HPM(:,:,ii) = I_MS(:,:,ii) .* (imageHR ./ (PAN_LP + eps));
end
