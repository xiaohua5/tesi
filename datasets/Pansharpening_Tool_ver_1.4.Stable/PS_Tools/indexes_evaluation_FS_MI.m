...
function [D_lambda,D_S,HQNR_index,SAM_index,sCC,D_S_star,Q_star,...
    ...
    indexes_evaluation_FS_MI(I_F,I_MS_LR,I_PAN,L,th_values,I_MS,...
...
D_S = D_s_decimated(I_F,I_MS,I_PAN,Qblocks_size,ratio,1,sensor_PAN,tag_PAN);
HQNR_index = (1-D_lambda)*(1-D_S);

I_PAN_filt = mean(imresize(I_PAN,1/ratio),3);
I_MS_LR_mean = mean(I_MS_LR,3);
I_PAN_mod = I_PAN*std2(I_MS_LR_mean)/std2(I_PAN_filt);
% I_PAN_mod = I_PAN*mad(I_MS_LR_mean(:))/mad(I_PAN_filt(:));
% D_S_mod = D_s_decimated_mod(I_F,I_MS,I_PAN_mod,Qblocks_size,ratio,1,sensor_PAN,tag_PAN);
D_S_mod = D_s_decimated(I_F,I_MS,I_PAN_mod,Qblocks_size,ratio,1,sensor_PAN,tag_PAN);
HQNR_index_mod = (1-D_lambda)*(1-D_S_mod);

D_S_star = D_s_Reg(I_F,mean(I_PAN,3));
Q_star = (1-D_lambda)*(1-D_S_star);
% [Q_star,~,D_S_star]= QStar(I_F,I_MS_LR,I_PAN,Qblocks_size,...
%     sensor_MS,tag_MS,ratio);
D_S_HSMS = D_s_decimated_HSMS(I_F,I_MS,I_PAN,Qblocks_size,ratio,1,...
    sensor_PAN,tag_PAN);
...
[~,D_S_Khan]=Khan(I_F,I_MS,mean(I_PAN_mod,3),I_MS_LR,sensor_MS,tag_MS,ratio,Qblocks_size);
