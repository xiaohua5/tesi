function Ds = D_s_decimated_HSMS_BS(fused,exp,ref,Groups_Ref,ass_Ref,...
...
% ref_filt = mean(ref_filt,3);
% ref = mean(ref,3); 
%ref_LP_d = ref_LP(3:ratio:end,3:ratio:end);
% hh = gaussfir(1,1,18); % Gaussian filter matching ref MTF
% % hh = gaussfir(1,1,20); % Gaussian filter matching ref MTF
% H = hh'*hh;
% w = size(H,1);
% ref_filt_full = imfilter(odd_extension(ref,w,w),H,'full');
% ref_filt = ref_filt_full(w:w+N-1,w:w+M-1);
% keyboard
Ds = 0;
for ii = 1:Nb
    band1 = fused(:,:,ii);
    ...
    band2 = imresize(band2,1./ratio,'nearest');%ref_filt;
%         C = cov(band1(:),band2(:))/cov(band2(:));
%         band2 = C(1,2)/C(2,2)*band2; 
%     band1 = exp(:,:,ii);
%         band2 = ref_filt;
