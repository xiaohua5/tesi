...
#    import pdb; pdb.set_trace()
    ...
    model['ratio']=4            # modifico       03/05
    return model
        


# solo esponenti di due
...
# this is interpGeneral
def interpGeneral(image, ratio):
...
#        return
    ...
    BaseCoeff_Matlab = sio.loadmat('BaseCoeff_General_ratio_3_SaoPaulo_Multi.mat')
    BaseCoeff_general = np.array(BaseCoeff_Matlab['BaseCoeff'])    
    
    BaseCoeff_2_Matlab = sio.loadmat('BaseCoeff_2_General_ratio_3_SaoPaulo_Multi.mat')
    BaseCoeff2_general = np.array(BaseCoeff_2_Matlab['BaseCoeff2'])
    
    BaseCoeff = BaseCoeff2_general[0,:]
    
    
#    CDF23 = 2*np.array([0.5, 0.305334091185, 0, -0.072698593239, 0, 0.021809577942, 0, -0.005192756653, 0, 0.000807762146, 0, -0.000060081482,0.5, 0.305334091185, 0, -0.072698593239, 0, 0.021809577942, 0, -0.005192756653, 0, 0.000807762146, 0])
#    d = CDF23[::-1] 
#    CDF23 = np.insert(CDF23, 0, d[:-1])
#    BaseCoeff = CDF23
#    a = np.transpose(BaseCoeff_general)
    
    I1LRU = np.zeros((b, ratio*r, ratio*c))
    
#    I1LRU[:, 1:I1LRU.shape[1]:2, 1:I1LRU.shape[2]:2]=image
    
#    I1LRU[: , math.floor(ratio/2)+1:I1LRU.shape[1]:ratio, math.floor(ratio/2)+1:I1LRU.shape[2]:ratio]=image

    I1LRU[: , (ratio/2)+1:I1LRU.shape[1]:ratio, (ratio/2)+1:I1LRU.shape[2]:ratio]=image
     ##I1LRU[math.floor(ratio/2)+1:ratio:end,math.floor(ratio/2)+1:ratio:end,:] = image;
