...
from others import interpGeneral
#from othersGeneral import interp23
#from others import interpGeneral
from fine_tuning_General import fine_tuning_General
...
def PNN_test_divisible_9 (I_MS_LR,I_PAN,inputImg, param,net,path,mode,epochs):    
    ...
    if epochs != 0:
        ...
        fine_tuning_General(I_MS_LR,I_PAN,param,epochs,FTnetwork_dir_out)        
    ...
    #input preparation with interpGeneral into interp23 in this case
    if param['typeInterp']=='interp23tap':
           
        I_MS = interpGeneral(I_MS_LR, 3)       
#        I_MS = interp23(I_MS_LR, param['ratio'])        
        
        if len(NDxI_LR)!=0:
            
            NDxI = interpGeneral(NDxI_LR, 3)
    else:
