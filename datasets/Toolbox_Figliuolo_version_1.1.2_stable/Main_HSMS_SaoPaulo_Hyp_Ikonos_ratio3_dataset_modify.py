...
parser.add_argument('-s', '--sensor', type=str, default='SAOPAULO_Multi',     #name dataset
...
#%% dataset modify for application to Wald's protocol
I_PAN = I_PAN[0:189,:,:]
...
#Groups_matlab = sio.loadmat('Groups_CEN_SaoPaulo_Hyp_Ikonos_ratio_3.mat')
#Groups = np.array(Groups_matlab['Groups'])

#%% Fusion data based on CNN
# Load Variables MANUAL4
Groups_Masi_matlab = sio.loadmat('Groups_MANUAL4_SaoPaulo_Hyp_Ikonos_ratio_3.mat')
Groups_Masi = np.array(Groups_Masi_matlab['Groups'])
Ass_vector_matlab = sio.loadmat('ass_vector_HS_MS_MANUAL_4_Multi_SaoPaulo_ratio_3.mat')
...
for ig in range(0,np.size(Groups_Masi)):  # start from zero
    ...
    HS_BS_Masi_Manual4 = Groups_Masi[0,ig]-1    # less one
    I_MS_g = I_MS[HS_BS_Masi_Manual4[0,:],:,:]    
    I_MS_LR_g = I_MS_LR[HS_BS_Masi_Manual4[0,:],:,:]    
    squeezeIndex = ass_vector[0][ig]-1
    I_PAN_g = np.squeeze(I_PAN[ :, :,squeezeIndex])
    I_GT_g = I_GT[HS_BS_Masi_Manual4[0,:],:,:]    
    
    I_Fus_IHS[HS_BS_Masi_Manual4[0,:],:,:]= IHS(I_MS_g,I_PAN_g)  
...
for ig in range(0,np.size(Groups_Masi)):  # start from zero
    ...
    I_Fus_Brovey[HS_BS_Masi_Manual4[0,:],:,:]= Brovey(I_MS_g,I_PAN_g)  
...
#
#tic = time.time()   
#for ig in range(0,np.size(Groups)):    # started from zero
#       
#    HS_BS = Groups[0,ig]-1          # less one
#    I_MS_g = I_MS[HS_BS[0,:],:,:]
...
for ig in range(0,np.size(Groups_Masi)):  # started from zero
    ...
    I_Fus_CNN[ HS_BS_Masi_Manual4[0,:], : , :] = PNN_test_divisible_9(I_MS_LR_g,I_PAN_g_modify,inputImg,PNN_model,net,path,mode,epochs) 
