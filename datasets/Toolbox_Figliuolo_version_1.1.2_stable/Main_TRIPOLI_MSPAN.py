...
parser.add_argument('-s', '--sensor', type=str, default='TRIPOLI', 
...
execfile('copy_xml_fields_testing.py') #for Python 2.7
...
I_PAN = np.array(inputImg['I_PAN_LR'],dtype='double')       
I_GT = np.array(inputImg['I_MS'],dtype='double').transpose(2,0,1)
L = 11
ratio = 4

#%% Load others data from matlab
I_MS_read = sio.loadmat('I_MS_Tripoli.mat')
I_MS = np.array(I_MS_read['I_MS'],dtype='double').transpose(2,0,1)
I_MS_LR_read = sio.loadmat('I_MS_LR_Tripoli.mat')
